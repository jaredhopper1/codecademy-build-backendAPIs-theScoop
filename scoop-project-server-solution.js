...
function debug(str, obj) {
    if (DEBUG) {
        ...
        if (obj) console.log(JSON.stringify(obj));
    }
}
...
function createArticle(url, request) {
  ...
  debug('createArticle ', requestArticle);
...
function createComment(url, request) {
 ...
 // make sure both the user and the article exist!
  if (requestComment && requestComment.body &&
    ...
    database.comments[thisComment.id] = thisComment;
    database.users[thisComment.username].commentIds.push(thisComment.id);
    database.articles[thisComment.articleId].commentIds.push(thisComment.id);

    response.body = {comment: thisComment};
    response.status = 201;
  } else {
    response.status = 400;
  }
  //debug('createComment END', response);
...
function deleteComment(url, request) {
  ...
  if (savedComment) {
    ...
    // remove comment ID from the author's comment IDs
    const userCommentIds = database.users[savedComment.username].commentIds;
    userCommentIds.splice(userCommentIds.indexOf(id), 1);

    // remove comment ID from the article's comment IDs
...
function loadDatabase() {
    ...
    try {
        let d = yaml.safeLoad(fs.readFileSync('./data.yml', 'utf8'));
        if (d) database = d;
        // debug(JSON.stringify(database, null, 4));
    } catch (e) {
        console.log(e);
    }
...
function saveDatabase() {
    ...
    // convert database to YAML string
    const ymlStr = yaml.safeDump(database);
    // write string to file
    fs.writeFileSync('./data.yml', ymlStr, (err) => {
        if (err) throw err;
        debug('File saved!');
        debug(ymlStr);
    });
